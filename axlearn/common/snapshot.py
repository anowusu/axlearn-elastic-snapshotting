...
class Snapshotter:
  ...
  def _worker(self):
    while True:
      pinned_state, step, input_iter_state = self._queue.get()
      try:
        _logger.info(
        ...
        with self._lock:
          self._latest_snapshot = (pinned_state, step, input_iter_state)
  ...
  def save_pytree(
      self, step: int, state: tree_types.PyTreeOf[jax.Array], input_iter_state: Any = None
  ) -> None:
    ...
    import copy
    try:
        # Try to deepcopy to prevent mutation from main thread
        input_iter_state_copy = copy.deepcopy(input_iter_state)
    except Exception:
        input_iter_state_copy = input_iter_state

    self._queue.put((pinned_state, step, input_iter_state_copy))

  def load_pytree(
      self,
      *,
      reset_snapshot_state: bool = True,
  ) -> tuple[tree_types.PyTree, Any]:
    ...
    Returns:
      A tuple of (restored array state, input_iter_state).
    ...
    with self._lock:
      ...
      pinned_state, step, input_iter_state = self._latest_snapshot
    ...
    if reset_snapshot_state:
      with self._lock:
        self._latest_snapshot = (host_target_state, step, input_iter_state)

    return restored_state, input_iter_state
  ...
  def latest(self) -> training.CheckpointMetadata[None] | None:
    ...
    with self._lock:
      if self._latest_snapshot is None:
        return None
      _, step, _ = self._latest_snapshot
